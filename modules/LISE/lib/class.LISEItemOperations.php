...
class LISEItemOperations {
    ...
    static final public function Save(LISE &$mod, LISEItem &$obj, $mleblock = "") {
        $arr = active_languages();
        if (count($arr) > 1) {
            if (!in_array($mod->_GetModuleAlias(), $arr)) {
                $mleblock = "";
            }
        }
        
        if($mleblock == '_en') $mleblock = "";
        ...
        if ($obj->item_id > 0) {
            ...
            $query = 'UPDATE ' . cms_db_prefix() . 'module_' . $mod->_GetModuleAlias() . '_item 
					SET `title'.$mleblock.'` = ?,`desc'.$mleblock.'` = ?, alias = ?, active = ?, start_time = ?, end_time = ?, modified_time = NOW(), key1 = ?, key2 = ?, key3 = ?, owner = ?, url = ? 
					WHERE item_id = ?';
...
//				die('FATAL SQL ERROR: ' . $db->ErrorMsg() . '<br/>QUERY: ' . $db->sql);
            ...
            $query = 'INSERT INTO ' . cms_db_prefix() . 'module_' . $mod->_GetModuleAlias() . '_item 
					(`title' . $mleblock . '`,`desc' . $mleblock . '`, alias, position, active, create_time, modified_time, start_time, end_time, key1, key2, key3, owner, url) 
					VALUES (?,?, ?, ?, ?, NOW(), NOW(), ?, ?, ?, ?, ?, ?, ?)';
...
// Init search
    ...
    static final public function Load(LISE &$mod, LISEItem &$obj, $row = false, $mleblock) {
        ...
        if ($row) {
            #Default MLE Language
//            $defaultMLE = get_site_preference('default_lang_mle');
//            $defaultMLE = explode("_", $defaultMLE);
            // Item table
            
            $obj->item_id = $row['item_id']; // deprecated
            $obj->title = ($mleblock !== '_en') ? $row['title'.$mleblock] : $row['title'];
            $obj->desc = ($mleblock !== '_en') ? $row['desc'.$mleblock] : $row['desc'];
