...
switch ($oldversion) 
{
 ...
 if( $this->GetPreference('allow_autoupdate', 0) )
 {
   ...
   $loaded = array_keys( $modops->GetLoadedModules() );

   foreach($modules as $module) 
  {
    if( in_array($module->module_name, $loaded) )
    {
      $inst = $modops->get_module_instance($module->module_name);
      
      if( $inst->AllowAutoUpgrade() )
      {
        $modops->UpgradeModule($module->module_name);
      }
    } 
  }
 }
