...
class FrontEndUsersManipulator extends UserManipulator
{
    ...
    public function add_history($uid,$message)
    {
        ...
        $dbr = $db->Execute($query,array($uid,session_id(),substr($message,0,255),$ip));
        return TRUE;
    }
...
// in order not to add it to the db
    ...
    function _cache_propertydefns()
    {
        if( !is_array($this->_cached_propdefns) ) {
            ...
            for( $i = 0; $i < count($data); $i++ ) {
                ...
                if( !empty($data[$i]['attribs']) ) $data[$i]['extra'] = unserialize($data[$i]['attribs']);
    ...
    function Login( $username, $password, $groups = '', $ignored = false, $force_logout = false)
    {
        ...
        $uid = $this->GetUserID( $username );
        if( !$uid ) return [ FALSE, $mod->Lang('error_usernotfound') ];
        if( $force_logout ) $this->Logout($uid);

        if( $this->IsAccountExpired( $uid ) ) return array(FALSE,$mod->Lang('error_accountexpired'));
        if( $mod->GetPreference('allow_repeated_logins') == 0 ) {
            ...
            if( $dbresult && $dbresult->RecordCount() ) return [ FALSE, $mod->Lang('error_norepeatedlogins') ];
        }
        ...
        audit($uid,$mod->GetName(),"$username logged in");
        $this->add_history($uid,'login');

        // send the event.
        $ip = \cge_utils::get_real_ip();
        $mod->SendEvent('OnLogin',array('id'=>$uid,'username'=>$username,'ip'=>$ip));
...
// returns an array
    function DeleteUserFull( $id )
    {
        ...
        try {
            ...
            // send the event... before we actually delete stuff.
            $parms = array();
            $parms['id'] = $id;
...
// userid api function
// returns array
    function SetGroup( $id, $name, $desc = null )
    {
        $id = (int) $id;
        $name = trim($name);
        $desc = trim($desc);
        ...
        if( $id < 1 || !$name ) {
            return array(FALSE,$mod->Lang('error_insufficientparams'));
        }
    ...
    function SetUserDisabled( $uid, $flag = TRUE )
    {
        ...
        if( $flag ) {
            ...
            $this->add_history($uid,'user disabled');
        }
        else {
    ...
    function ForcePasswordChange( $uid, $flag = TRUE )
    {
        ...
        if( $flag ) $this->add_history($uid,'forcepwreset');
    }
    ...
    function ForceChangeSettings( $uid, $flag = TRUE )
    {
        ...
        if( $flag ) $this->add_history($uid,'forcechsettings');
    }
    ...
    function ForceVerify( $uid, $flag = TRUE )
    {
        ...
        $dbr = $db->Execute($query,array($flag,$uid));
        //if( !$dbr ) throw new \LogicException('problem setting force verify flag for user '.$uid);

        if( $flag ) $this->add_history($uid,'init_verify');
    }
...
//
    ...
    public function CheckPassword($username,$password,$groups = '')
    {
        ...
        //$where[] = 'COALESCE(u.must_validate,0) = 0';
    ...
    {
        // note, no user effect if rememberme is enabled and user has the cookie.
        if( !$uid ) return;
        $db = $this->GetDb();
...
// old userid api function
    ...
    private function _get_saved_logindetails()
    {
        ...
        if( $lastr > 0 && $lastr < time() - 180 ) $do_read = TRUE;

        if( !$do_read ) {
            // see if uid info is in the session
            if( !empty($_SESSION[$key]) ) $data = unserialize(base64_decode($_SESSION[$key]));
        }

        if( !$do_read && !$data ) {
            // get data from a session cookie
            if( $module->GetPreference('cookie_keepalive',0) && !empty($_COOKIE[$key]) ) $data = unserialize(base64_decode($_COOKIE[$key]));
        }

        if( !$do_read && !$data ) {
            // see if we can get the data from a permanent cookie.
            if( ($cn = $module->GetPreference('cookiename')) != '') {
                // this is remember me functionality. as opposed to a session cookie.
                $tmp = \cms_cookies::get($cn);
                if( $tmp ) {
    ...
    private function _set_saved_logindetails($uid)
    {
        ...
        if( $module->GetPreference('cookie_keepalive',0) ) {
            ...
            if( $user_session_expires ) \cms_cookies::set($key,base64_encode(serialize($data)),time()+$user_session_expires); // expiry to 24 hours.
        }
    ...
    private function _clear_logindetails($uid,$session_only = FALSE)
    {
        ...
        \cms_cookies::erase($key);
        $feu = \cms_utils::get_module(MOD_FRONTENDUSERS);
        if( ($cn = $feu->GetPreference('cookiename')) != '' ) {
            \cms_cookies::erase($cn);
        }
