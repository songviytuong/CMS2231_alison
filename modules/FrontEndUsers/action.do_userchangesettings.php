...
if( isset($params['feu_cancel']) ) {
    if( $returnto ) {
        $returnid = $this->resolve_alias_or_id($returnto,$returnid);
...
if( $consumer->has_capability(feu_auth_consumer::CAPABILITY_CHANGEPASSWD) ) {
    ...
    if( $uinfo['force_newpw'] && !$password ) {
        $params['error'] = 1;
        $params['message'] = $this->Lang('error_requiredfield','password');
        $this->myRedirect($id, 'changesettings', $returnid, $params );
    }
    if( $uinfo['force_newpw'] && !$repeat ) {
        $params['error'] = 1;
        $params['message'] = $this->Lang('error_requiredfield','repeatpassword');
        $this->myRedirect($id, 'changesettings', $returnid, $params );
    }
...
$this->ForceChangeSettings($uid,FALSE);
...
else if( ($tplstr = $this->GetPreference('pageid_afterchangesettings')) ) {
    $tpl = $smarty->CreateTemplate('string:'.$tplstr);
    $tpl->assign('username',$uinfo['username']);
    $tpl->assign('userinfo',$uinfo);
...
$this->myRedirect('cntnt01','logoutform',$returnid,$parms);
