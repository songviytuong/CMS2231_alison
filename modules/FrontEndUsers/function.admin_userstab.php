...
$tpl = $this->CreateSmartyTemplate('userlist.tpl');
if( $this->_HasSufficientPermissions('listusers') ) {
    ...
    if( isset( $params['dobulk']) ) {
        if( isset($params['selected']) && is_array($params['selected']) ) {
            ...
            $this->myRedirect( $id, 'admin_bulkactions', $returnid, array('job'=>\cge_param::get_string($params,'bulk_action','delete'),'uids'=>$sel));
        }
    }
    ...
    $tpl->assign( 'perm_removeusers', $this->_HasSufficientPermissions('removeusers')?1:0);
    ...
    $tpl->assign('emailtext', $this->Lang('email'));
    $tpl->assign('statustext', $this->Lang('status'));
    $tpl->assign('createdtext', $this->Lang('created'));
    $tpl->assign('expirestext', $this->Lang('expires'));
    if( is_array($users) ) {
        foreach( $users as $row ) {
            $onerow = new StdClass();
            $onerow->id = $row['id'];
            $onerow->disabled = (bool) $row['disabled'];
            $onerow->must_validate = (bool) (isset($row['must_validate'])) ? $row['must_validate'] : false;
            $onerow->created  = $row['createdate'];
            $onerow->username = $this->CreateLink($id, 'admin_edituser', $returnid, $row['username'], array('user_id' => $row['id']));
            ...
            if( $this->_HasSufficientPermissions('editusers') ) {
                $onerow->editlink =
                    $this->CreateLink ($id, 'admin_edituser', $returnid,
...
if( $this->_HasSufficientPermissions('users') ) {
    if( count($groups) ) $tpl->assign('import_url',$this->create_url($id,'admin_import_users',$returnid));
    if( count($rowarray) ) $tpl->assign('export_url',$this->create_url($id,'admin_export_users',$returnid));
}
