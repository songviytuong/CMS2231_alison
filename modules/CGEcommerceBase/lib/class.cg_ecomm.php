...
final class cg_ecomm
{
    ...
    {
        $res = null;
        if( self::is_supplier_module($source) ) $res = cge_utils::get_module($source);
        return $res;
    }
    ...
    public static function &get_handling_module()
    {
        $res = null;
        $ecomm = cge_utils::get_module('CGEcommerceBase');
        $modname = $ecomm->GetPreference('handling_module');
        if( $modname == '' || $modname == '-1' ) return $res;
    ...
    {
        $mod = cge_utils::get_module(MOD_CGECOMMERCEBASE);
        $tmp = $mod->GetPreference('sel_payment_modules');
        if( $tmp ) return explode(',',$tmp);
    }
    ...
    public static function get_product_info($source,$product_id)
    {
        ...
        if( $module && method_exists($module,'get_product_info') ) {
            ...
            if( is_object($tmp) && ($tmp instanceof cg_ecomm_productinfo) ) return $tmp;
        }
        ...
        if( is_array($suppliers) && count($suppliers) ) {
            foreach( $suppliers as $supplier ) {
                try {
                    ...
                    if( is_object($res) && ($res instanceof cg_ecomm_productinfo) ) return $tmp;
                }
                catch( Exception $e ) {
                    // skip it.
                }
    ...
    public static function get_displayable_option(cg_ecomm_productinfo_option $opt)
    {
...
//   }
    ...
    public static function get_payment_cartitem_policy()
    {
        ...
        if( !is_array($module_list) || !count($module_list) ) return $policy;
        ...
        foreach( $module_list as $module_name ) {
            ...
            if( !is_object($policy) ) return; // no policy.

            $policy->merge($m_policy,FALSE);
        }
        return $policy;
    }
