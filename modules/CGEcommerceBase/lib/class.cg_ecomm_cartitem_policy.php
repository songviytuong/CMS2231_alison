...
class cg_ecomm_cartitem_policy
{
    ...
    public function merge(cg_ecomm_cartitem_policy& $in,$minimalist = TRUE)
    {
        if( $minimalist ) {
            ...
            if( !$this->_mixed_subscriptions ||	!$in->_mixed_subscriptions ) $this->_mixed_subscriptions = 0;
        }
        else {
            ...
            if( $this->_mixed_subscriptions || $in->_mixed_subscriptions ) $this->_mixed_subscriptions = 1;
        }
    }

    public function matches($n_products, $n_services, $n_subscriptions)
    {
        if( $this->_max_products >= 0 && $n_products > $this->_max_products ) return FALSE;
        if( $this->_max_services >= 0 && $n_services > $this->_max_services ) return FALSE;
        if( $this->_max_subscriptions >= 0 && $n_subscriptions > $this->_max_subscriptions ) return FALSE;

        if( !$this->_mixed_subscriptions ) {
            if( $n_subscriptions > 0 && $n_products > 0 ) return FALSE;
        }
        return TRUE;
    }
}
