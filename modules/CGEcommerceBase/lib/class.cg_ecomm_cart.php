...
class cg_ecomm_cart
{
   ...
   * @return bool
   */
  public static function check_cartitem_valid($existing_items, cg_ecomm_cartitem& $new_item, &$message = '')
  {
      // 1.  Build a policy
      ...
      foreach( $existing_items as &$one_item ) {
          ...
          if( ($tmp = $one_item->get_subscription()) ) {
              if( $tmp->get_payperiod() != cg_ecomm_productinfo_subscription::SUBSCR_PERIOD_NONE ) $have_subscriptions++;
          }
      }
      ...
      if( !$policy->handle_products() && $have_products ) return FALSE;
      if( !$policy->handle_services() && $have_services ) return FALSE;
      if( !$policy->handle_subscriptions() && $have_subscriptions ) return FALSE;
      $tmp = $policy->max_subscriptions();
      if( $tmp > 0 && $tmp < $have_subscriptions ) return FALSE;
  ...
  public static function calculate_cartitem_summary($product_info,cg_ecomm_cartitem $item)
  {
