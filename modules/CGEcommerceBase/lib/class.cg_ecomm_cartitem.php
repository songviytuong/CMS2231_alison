...
class cg_ecomm_cartitem
{
    ...
    private $_base_price = null; // the base price of the object, not including options
    ...
    private $_item_total = null; // only set if the unit total has been overriden.... accounts for quantity as well.
    ...
    {
        if( $this->_unit_discount ) return $this->_unit_discount;
        return 0;
    }
    ...
    {
        if( is_array($attribs) ) $this->_attributes = $attribs;
    }
    ...
    public function to_array()
    {
        ...
        if( $this->_subscription ) $tmp['subscription'] = $this->_subscription->to_array();
        return $tmp;
    }

    public function __tostring()
    {
        return implode('//',array($this->get_source(),$this->get_sku(),$this->get_summary(),$this->get_unit_price()));
    }

    public function compare(cg_ecomm_cartitem $other_item)
    {
        $tmp1 = $this->to_array();
