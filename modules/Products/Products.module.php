...
final class Products extends CGExtensions
{
    ...
    public function GetName() { return 'Products'; }
    public function GetFriendlyName() { return $this->Lang('product_manager'); }
    public function AllowAutoInstall() { return FALSE; }
    public function AllowAutoUpgrade() { return FALSE; }
    public function IsPluginModule() { return true; }
    public function HasAdmin() { return true; }
    public function GetVersion() { return '2.26.9'; }
    ...
    {
        return array('CGExtensions'=>'1.55.3', 'CGSimpleSmarty'=>'2.1.6', 'JQueryTools'=>'1.4');
    }
...
EOT;
    ...
    public function InitializeFrontend()
    {
        ...
        if( $hierpage <= 0 ) $hierpage = $detailpage;
    ...
    {
        static $obj;
        if( !$obj ) $obj = new \Products\ProductStorage( $this, \cge_utils::get_db() );
        return $obj;
    }
    ...
    function &GetCategory($category_id,$full = false)
    {
        ...
        if( $full ) {
            ...
            if( is_array($tmp2) ) $onerow->data = $tmp2;
        }
    ...
    {
        if( $product->status != $product::STATUS_PUBLISHED ) return [];
        ...
        foreach( $fieldefs as $onedef ) {
            if( $onedef->public ) continue;
            if( !isset($fieldvals[$onedef->id]) ) continue;

            switch( $onedef->type ) {
            case 'textbox':
            case 'textarea':
            case 'dropdown':
                if( isset($onedef->value) ) $results[] = $onedef->value;
                break;
            }
        }
    ...
    {
        $product = $this->GetStorage()->get_by_id( $id );
        return $product->name;
    }
    ...
    {
        if( $inline || !product_utils::can_do_pretty($action,$params) ) return;
        $prettyurl = null;
        ...
        case 'default':
            if( isset($params['categoryid']) ) {
                ...
                $prettyurl = sprintf("%s/bycategory/%d/%d",
                                     $this->GetPreference('urlprefix','products'), (int)$params['categoryid'],$returnid);
                if( isset($params['categoryname']) ) $prettyurl .= '/'.munge_string_to_url($params['categoryname']);
            }
        ...
        case 'hierarchy':
            ...
            if( !$nochildren ) {
                ...
                if( $this->GetPreference('prettyhierurls',0) ) {
                    ...
                    if( is_array($info) ) $prettyurl .= '/'.munge_string_to_url($info['name']);
                }
                break;
            }
        } // switch
    ...
    function get_product_info($product_id)
    {
        ...
        if( !$product ) return;
        if( $product->status != $product::STATUS_PUBLISHED ) throw new \LogicException('Cannot get ecomm product information for an unpublished product');
    ...
    {
        if( $this->_price_adjuster ) return;
    ...
    {
        $value = (float) $value;
        if( $this->_price_adjuster ) return $this->_price_adjuster->get_adjusted_price( $product, $value );
        return $value;
    }
} // class
