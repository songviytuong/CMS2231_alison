...
if (isset($params['submit'])) {
    try {
        ...
        if( $row['id'] < 1 ) {
            ...
            $sql = 'INSERT INTO '.cms_db_prefix().'module_products_fielddefs (name, prompt, type, max_length, item_order, create_date, modified_date)
                    VALUES (?,?,?,?,?,NOW(),NOW())';
            $dbr = $db->Execute( $sql, [ $row['name'], $row['prompt'], $row['type'], $row['max_length'], $max ] );
