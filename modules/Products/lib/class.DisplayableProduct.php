...
class DisplayableProduct extends Product
{
    ...
    protected function pretty_detail_url()
    {
        ...
        if( $this->url ) {
            ...
            $done = false;
            if( $module->GetPreference('usehierpathurls',0) && !empty($product['alias']) && (isset($product['hierarchy_id']) && $product['hierarchy_id'] > 0) ) {
                $hier_id = self::get_product_hierarchy_id($this->id);
                if( $hier_id ) {
                    $tmp = hierarchy_ops::get_hierarchy_info($this->first_hierarchy);
                    if( $tmp ) {
