...
class products_query extends \CGExtensions\query\query
{
    private static $_keys = array('productid','product_id','productlist','alias','category','categoryid','excludecat',
                                  ...
                                  'productnamelike','desclike','sortby','sorttype','limit','offset','status','pricerange','useor','nochildren',
                                  'owner');
    ...
    {
        if( in_array($test,self::$_keys) ) return TRUE;
        return FALSE;
    }

    public function __construct($data = array())
    {
        if( is_array($data) ) $this->_origparms = $data;
        $this->reset();
    }
  ...
  {
      if( $key == 'pagelimit' ) $key = 'limit';
      if( !in_array($key,self::$_keys) ) throw new Exception('Get: invalid key '.$key.' specified for products_query');
  ...
  {
      if( $key == 'pagelimit' ) $key = 'limit';
      if( !in_array($key,self::$_keys) ) throw new Exception('Set: Invalid Key '.$key.' specified for products_query');
      if( !$value ) return;
      ...
      case 'pricerange':
          if( strpos($value,':') !== FALSE ) $this->_data[$key] = $value;
          break;
      ...
      case 'productlist':
          $this->_resetidentifiers();
          $tmp = $value;
          if( is_string($value) ) $tmp = explode(',',$value);
      ...
      case 'sortby':
          ...
          default:
              if( startswith($value,'f:') ) {
                  ...
                  if( is_array($fielddefs) && isset($fielddefs[$fieldname]) ) $this->_data[$key] = $value;
              }
              else {
  ...
  {
      if( !in_array($key,self::$_keys) ) throw new Exception('Exists: Invalid Key '.$key.' specified for products_query');
      return isset($this->_data[$key]);
  }

  public function offsetUnset($key)
  {
      if( !in_array($key,self::$_keys) ) throw new Exception('Unset: Invalid Key '.$key.' specified for products_query');
      return FALSE;
  }
