...
class ProductStorage
{
    ...
    {
        if( self::$_inst > 0 ) throw new \LogicException('Only one instance of '.__CLASS__.' is allowed');
        self::$_inst++;
        $this->_mod = $mod;
        $this->_db = $db;
    }
    ...
    private function _insert_product( Product $product )
    {
        ...
        $sql = 'INSERT INTO '.self::product_table_name().' (product_name, price, details, taxable, status, weight, sku, alias, url, owner,
                   create_date, modified_date)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?)';
        $i_arr = [ $product->name, $product->price, $product->details, $product->taxable, $product->status, $product->weight, $product->sku, $product->alias, $product->url,
                   $product->owner,
                   $adjust_parms['create_date'], $adjust_parms['modified_date'] ];
        $dbr = $db->Execute( $sql, $i_arr );
        if( !$dbr ) throw new \cg_sql_error( 'SYSTEM ERROR: Problem inserting product: '.$db->ErrorMsg() );
    ...
    private function _delete_connected_data( $product_id, $delete_files = false )
    {
        ...
        if( $product_id < 1 ) throw new \LogicException('Invalid product_id passed to '.__METHOD__);
    ...
    private function _update_product( Product $product )
    {
        ...
        $sql = 'UPDATE '.self::product_table_name().' SET product_name = ?, price = ?, details = ?, taxable = ?, status = ?, weight = ?,
                sku = ?, alias = ?, url = ?, owner = ?, modified_date = ? WHERE id = ?';
        $adj_parms = [ 'modified_date'=>$this->db_datestring() ];
        $db->Execute( $sql, [ $product->name, $product->price, $product->details, $product->taxable, $product->status, $product->weight,
                              $product->sku, $product->alias, $product->url, $product->owner, $adj_parms['modified_date'], $product->id] );
    ...
    protected function check_product_name( Product $product )
    {
        ...
        if( !$product_name ) throw new \RuntimeException( $this->_mod->Lang('error_product_namerequired') );
        ...
        if( $t_id > 0 ) throw new \RuntimeException($this->_mod->Lang('error_product_nameused'));
        return $product;
    }
    ...
    public function check_product_alias( Product $product )
    {
        ...
        if( \product_ops::check_alias_used( $alias, $product->id ) ) throw new \RuntimeException($this->_mod->Lang('error_product_aliasused') );
    ...
    {
        if( empty($product->sku) && $this->_mod->GetPreference('skurequired') ) throw new \RuntimeException($this->_mod->Lang('error_product_skurequired'));
        if( !empty($product->sku) ) {
            if( \product_ops::check_sku_used( $product->sku, $product->id ) ) throw new \RuntimeException($this->_mod->Lang('error_product_skuused'));
        }
        return $product;
    }
    ...
    public function check_urlslug( Product $product )
    {
        ...
        if( $urlslug ) {
            ...
            if( strtolower($tr) != strtolower($urlslug) ) throw new \RuntimeException($this->_mod->Lang('error_product_invalidurlslug'));
    ...
    public function check_attribs( Product $product )
    {
        ...
        if( !is_array($attribs) || !count($attribs) ) return $product;
        ...
        foreach( $attribs as $attrib ) {
            ...
            $sku = trim($attrib->sku);
            if( $text && !in_array($text,$names) ) $names[] = $text;
            if( $sku && in_array($sku,$skus) ) throw new \RuntimeException($this->_mod->Lang('error_attrib_duplicatesku'));
            $skus[] = $sku;
        }
        if( count($names) < count($attribs) ) throw new \RuntimeException($this->_mod->Lang('error_attrib_text'));

        foreach( $attribs as $attrib ) {
            if( empty($product->sku) ) {
                if( $this->_mod->GetPreference('skurequired') ) throw new \RuntimeException($mod->Lang('error_attrib_skurequired'));
            }
            else {
                if( \product_ops::check_sku_used( $attrib->sku, $product->id ) ) throw new \RuntimeException($this->_mod->Lang('error_attrib_skuused'));
            }
        }
        return $product;
    }
    ...
    protected function check_fields( Product $product )
    {
        $get_fielddef = function( $fid, array $fielddefs ) {
            foreach( $fielddefs as $one ) {
                if( $one->id == $fid ) return $one;
            }
        };

        $fielddefs = \product_utils::get_fielddefs(TRUE,TRUE,FALSE);
        if( !count($fielddefs) && count($product->field_vals) ) throw new \LogicException('No fielddefs defined, but have field values in this product');

        foreach( $product->field_vals as $fid => $val ) {
            if( !$get_fielddef($fid,$fielddefs) ) throw new \LogicException("Fielddef with id $fid exists in product but is not defined");
    ...
    {
        if( !$product->name ) throw new \RuntimeException( $this->_mod->Lang('error_product_nameused') );
    ...
    {
        if( !isset($row['id']) || $row['id'] < 1 || !isset($row['product_name']) || !$row['product_name'] ) return;
        ...
        if( is_array($cats) ) $row['categories'] = $cats;
        ...
        if( is_array($hiers) ) $row['hierarchies'] = $hiers;
        ...
        if( is_array($fv) ) {
            ...
            foreach( $fv as $one ) {
                $fid = $one['fielddef_id'];
                if( $fid < 1 ) continue;
                if( $defs[$fid]->type == 'subscription' || $defs[$fid]->type == 'dimensions' ) $one['value'] = unserialize($one['value']);
        ...
        $row['attribs'] = [];
        if( is_array( $tmp ) ) $row['attribs'] = $tmp;
    ...
    {
        $name = trim($name);
        if( !$name ) throw new \LogicException('Invalid product name passed to '.__METHOD__);
    ...
    public function add_to_cache( Product $product )
    {
        ...
        if( !isset(self::$_cache[$product->id]) ) self::$_cache[$product->id] = $product;
    }
    ...
    public function get_by_id( $product_id, $displayable = false )
    {
        ...
        if( isset(self::$_cache[$product_id]) ) {
            ...
            if( $res ) $this->add_to_cache( $res );
        }
        if( $res && $displayable ) $res = new DisplayableProduct( $res );
        return $res;
    }
    ...
    {
        $sku = trim($sku);
        if( !$sku ) throw new \LogicException('Invalid sku passed to '.__METHOD__);

        $sql = 'SELECT id FROM '.self::product_table_name().' WHERE sku = ?';
        $pid = (int) $this->_db->GetOne( $sql, [ $sku ] );
        if( $pid ) return self::get_by_id( $pid, $displayable );
    ...
    public function delete_by_id( $product_id )
    {
        ...
        if( isset(self::$_cache[$product_id]) ) unset(self::$_cache[$product_id]);
        $db = $this->_db;
    ...
    public function get_product_owners( array $product_ids )
    {
        ...
        while( !$rs->EOF ) {
            ...
            $owner = (int) $row['owner'];
            if( $owner < 1 ) continue;
            if( !count($out) || !in_array( $owner, $out) ) $out[] = $owner;
            $rs->MoveNext();
        }
        return $out;
    }

    public static function product_table_name() { return CMS_DB_PREFIX.'module_products'; }
    public static function product_categories_table_name() { return CMS_DB_PREFIX.'module_products_product_categories'; }
    public static function product_hierarchies_table_name() { return CMS_DB_PREFIX.'module_products_prodtohier'; }
    public static function product_fieldvals_table_name() { return CMS_DB_PREFIX.'module_products_fieldvals'; }
    public static function product_attribs_table_name() { return CMS_DB_PREFIX.'module_products_attribs'; }
