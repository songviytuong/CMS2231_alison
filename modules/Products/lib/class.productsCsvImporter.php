...
final class productsCsvImporter
{
    ...
    private function _handleProductLine()
    {
        ...
        $iquery = 'INSERT INTO '.cms_db_prefix().'module_products_attribs (product_id, text, adjustment, sku, qoh) VALUES (?,?,?,?,?)';
        for( $attr_idx = 1; $attr_idx <= 20; $attr_idx++ ) {
            ...
            foreach( $attr_key_list as $attr_key ) {
                ...
                $rec[$attr_key] = trim($this->_getField($field_name));
            }
            $db->Execute($iquery,[ (int) $product_id, $rec['text'], $rec['adjustment'], $rec['sku'], (int) $rec['qoh'] ] );
        }
