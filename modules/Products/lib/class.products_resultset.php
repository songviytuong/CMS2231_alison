...
final class products_resultset extends \CGExtensions\query\resultset
{
    ...
    {
        if( strpos($str,'*') === FALSE && strpos($str,'%') === FALSE ) $str = '%'.$str.'%';
        $str = str_replace('*','%',$str);
        $str = str_replace('%%','%',$str);
        return $str;
    }
    ...
    protected function _query()
    {
        ...
        if( isset($obj['hierarchy']) && $obj['hierarchy'] != '' ) {
            ...
            if( isset($obj['nochildren']) && $obj['nochildren'] ) {
                ...
                $str = '**|junk|**';
                if( is_array($info) ) $str = $info['long_name'].'%';
        ...
        if( isset($obj['category']) && $obj['category'] != '' ) {
            ...
            if( is_array($icats) && count($icats) ) {
                foreach( $icats as $one ) {
                    $one = trim($one);
                    if( isset($ocats[$one]) ) $icatids[] = (int)$ocats[$one]->id;
                }
        ...
        if( isset($obj['excludecat']) && $obj['excludecat'] != '' ) {
            ...
            foreach( $ecats as $ecat ) {
                if( (int)$ecat > 0 ) {
                    ...
                    if( isset($ocats[$ecat]) ) $exclude_ids[] = (int)$ocats[$ecat]->id;
                }
            }
        ...
        //
        $expr = ' AND ';
        if( isset($obj['useor']) && $obj['useor'] ) $expr = ' OR ';
        if( count($joins) ) $query .= ' LEFT JOIN '.implode(' LEFT JOIN ',$joins);
        if( count($where) ) $query .= ' WHERE '.implode($expr,$where);
        if( $obj['sorttype'] == '' ) {
    ...
    private function _preload_attributes($product_ids)
    {
        ...
        if( !is_array($attribs) || count($attribs) == 0 ) return;
    ...
    private function _preload()
    {
        ...
        if( is_array($allcats) && count($allcats) ) $this->_preload_categories($product_ids);

        $this->_preload_hierarchies($product_ids);

        // preload field info
        $allfields = product_utils::get_fielddefs(TRUE,TRUE);
        if( is_array($allfields) && count($allfields) ) $this->_preload_fields($product_ids);
    ...
    public function __get($key)
    {
        ...
        if( $key == 'detailpage' ) {
            $detailpage = null;
            if( $this->_detailpage ) $detailpage = $mod->resolve_alias_or_id( $detailpage );
        ...
        if( $key == 'detailtemplate' ) return $this->_detailtemplate;
        if( $key == 'hierpage' ) {
        ...
        if( $key == 'curpage' ) return $this->_curpage;
        if( $key == 'notpretty' ) return $this->_notpretty;

        $this->_query();
        if( $key == 'EOF' ) return $this->_rs->EOF();
        if( $key == 'totalrows' ) return $this->_totalmatching;
        if( $key == 'fields' && $this->_rs && !$this->_rs->EOF() ) return $this->_rs->fields;
        if( $key == 'numpages' ) return ceil($this->_totalmatching / $this->_filter['pagelimit']);
    }
    ...
    public function __set($key,$value)
    {
        ...
        case 'detailtemplate':
            ...
            if( $this->_notpretty == '' ) $this->_notpretty = 'details';
            break;
    ...
    public function get_product($as_array = FALSE)
    {
        ...
        if( is_array($this->_category_cache) && count($this->_category_cache) ) {
            ...
            for( $i = 0; $i < count($this->_category_cache); $i++ ) {
                if( $this->_category_cache[$i]['product_id'] < $product_id ) continue;
                if( $this->_category_cache[$i]['product_id'] > $product_id ) break;
        ...
        if( is_array($this->_hierarchy_cache) && count($this->_hierarchy_cache) ) {
            ...
            for( $i = 0; $i < count($this->_hierarchy_cache); $i++ ) {
                if( $this->_hierarchy_cache[$i]['product_id'] < $product_id ) continue;
                if( $this->_hierarchy_cache[$i]['product_id'] > $product_id ) break;
        ...
        if( is_array($this->_field_cache) && count($this->_field_cache) ) {
            ...
            foreach( $flddefs as $onedef ) {
                if( $onedef->type == 'dimensions' || $onedef->type == 'subscription' ) $unserialize[] = $onedef->id;
            }
            ...
            for( $i = 0; $i < count($this->_field_cache); $i++ ) {
                ...
                if( in_array( $row['fielddef_id'], $unserialize ) ) $row['value'] = unserialize($row['value']);
                $fldarray[$row['fielddef_id']] = $row['value'];
            }
        ...
        if( $as_array ) return $product_arr;
        $mod = cms_utils::get_module('Products');
