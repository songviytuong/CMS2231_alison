...
<div class="c_full cf">
  <div class="grid_7">
    <div class="c_ful cfl">
      <div class="grid_2">{$mod->Lang('name')}:</div>
      <div class="grid_10">
        <input type="text" id="product_name" name="{$actionid}product_name" value="{$product_name}" size="40" maxlength="255"/>
      </div>
    </div>
    <div class="c_full cf">
      <div class="grid_2">{$mod->Lang('price')}:</div>
      <div class="grid_10">
        <input type="text" id="product_price" name="{$actionid}price" value="{$price}" size="10" maxlength="12"/>{$currency_symbol}
        <br/>{$mod->Lang('info_decimal_units')}</div>
    </div>
    <div class="c_full cf">
      <div class="grid_2">{$detailstext}:</div>
...
{cge_tabcontent_start name='fields'}
  {foreach from=$customfields item=customfield}
    <div class="c_full cf">
        <p class="grid_3 text-right">{if isset($customfield->prompt)}{$customfield->prompt}{else}{$customfield->name}{/if}:<br/>
          {if $customfield->type == 'image' && !empty($customfield->url)}
          {*<input type="text" placeholder="ALT"/>*}
              <a href="{$customfield->url}" class="fancybox">{admin_icon icon='picture-icon-24.png' class='thumb' title='Thumbnail'}</a>
...
$(document).ready(function(){
  ...
  <div class="c_full cf">
    ...
    <div class="grid_9">{$inputtaxable}</div>
  </div>
  <hr/>
